...
function init() {
  ...
  $(".btn-guardar").on("click", function (e) { if ( $(this).hasClass('send-data')==false) { $("#submit-form-docente").submit(); }  });
...
//Función Listar
function listar() {
	tabla_docente = $('#tabla-docente').dataTable({
    lengthMenu: [[ -1, 5, 10, 25, 75, 100, 200,], ["Todos", 5, 10, 25, 75, 100, 200, ]],//mostramos el menú de registros a revisar
    "aProcessing": true,//Activamos el procesamiento del datatables
    "aServerSide": true,//Paginación y filtrado realizados por el servidor
    dom:"<'row'<'col-md-3'B><'col-md-3 float-left'l><'col-md-6'f>r>t<'row'<'col-md-6'i><'col-md-6'p>>",//Definimos los elementos del control de tabla
    buttons: [
      { text: '<i class="fa-solid fa-arrows-rotate"></i> ', className: "buttons-reload btn btn-outline-info btn-wave ", action: function ( e, dt, node, config ) { if (tabla_docente) { tabla_docente.ajax.reload(null, false); } } },
      ...
      { extend: 'excel', exportOptions: { columns: [0,8,9,10,4,5,11,12,6,7], }, title: 'Lista de Docentes', text: `<i class="far fa-file-excel fa-lg" ></i>`, className: "btn btn-outline-success btn-wave ", footer: true,  }, 
      { extend: 'pdf', exportOptions: { columns: [0,8,9,10,4,5,11,12,6,7], }, title: 'Lista de Docentes', text: `<i class="far fa-file-pdf fa-lg"></i>`, className: "btn btn-outline-danger btn-wave ", footer: false, orientation: 'landscape', pageSize: 'LEGAL',  },
      { extend: "colvis", text: `<i class="fas fa-outdent"></i>`, className: "btn btn-outline-primary", exportOptions: { columns: "th:not(:last-child)", }, },
    ],
		"ajax":	{
			url: '../ajax/catedraticos.php?op=listar_tabla_principal',
			type: "get",
			dataType: "json",
			error: function (e) {
    ...
    columnDefs: [
      ...
      // { targets: [8,9, 10,11,12,],  visible: false,  searchable: false,  }, ocultar columnas
    ],
	}).DataTable();
}
//Función para guardar o editar

function guardar_y_editar_docente(e) {
	//e.preventDefault(); //No se activará la acción predeterminada del evento
	
	var formData = new FormData($("#form-agregar-docente")[0]);

	$.ajax({
		url: "../ajax/catedraticos.php?op=guardar_y_editar",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
        ...
        if (e.status == true) {	
					tabla_docente.ajax.reload(null, false);          
					show_hide_form(1)
					sw_success('Exito', 'Catedrático guardado correctamente.');
				} else {
					ver_errores(e);
				}				
      ...
      $(".btn-guardar").html('<i class="ri-save-2-line label-btn-icon me-2" ></i> Guardar').removeClass('disabled send-data');
		...
		xhr: function () {
			...
			xhr.upload.addEventListener("progress", function (evt) {
				if (evt.lengthComputable) {
					...
					$("#barra_progress_docente").css({ "width": percentComplete + '%' });
					$("#barra_progress_docente div").text(percentComplete.toFixed(2) + " %");
				}
			}, false);
			return xhr;
		},
		beforeSend: function () {
			...
			$("#barra_progress_docente").css({ width: "0%", });
			$("#barra_progress_docente div").text("0%");
      $("#barra_progress_docente_div").show();
		},
		complete: function () {
			$("#barra_progress_docente").css({ width: "0%", });
			$("#barra_progress_docente div").text("0%");
      $("#barra_progress_docente_div").hide();
		},
...
function desactivar(idpersonal_pi, idpersona, nombres) {
	$('.tooltip').remove();
	crud_eliminar_papelera(
    `../ajax/catedraticos.php?op=papelera&idpersona=${idpersona}`,
    `../ajax/catedraticos.php?op=eliminar&idpersona=${idpersona}`, 
    idpersonal_pi,  
    "!Elija una opción¡", 
    ...
    function(){ tabla_docente.ajax.reload(null, false);     },
    false, 
    false, 
    false,
    false
...
$(function () {
  $("#form-agregar-docente").validate({
    ignore: "",
    rules: {           
      tipo_documento:           { required: true, minlength: 1, maxlength: 2, },       
      numero_documento:    			{ required: true, minlength: 8, maxlength: 20, },       
      idcargo_personal:     		{ required: true, },       
      nombres:    		          { required: true, minlength: 4, maxlength: 200, },       
      apellidos:                { required: true, minlength: 4, maxlength: 200, },          
      
    },
    messages: {     
      tipo_documento:    			  { required: "Campo requerido", },
      numero_documento:    			{ required: "Campo requerido", }, 
      idcargo_personal:     		{ required: "Campo requerido", }, 
    ...
    submitHandler: function (e) {
      ...
      guardar_y_editar_docente(e);      
    },
  });
  $('#tipo_documento').rules('add', { required: true, messages: {  required: "Campo requerido" } });
  $('#idcargo_personal').rules('add', { required: true, messages: {  required: "Campo requerido" } });
});
